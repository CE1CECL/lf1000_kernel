...
static const enum gpio_resource reset_pins[MAX_CHANNELS] = {
	SD0_RESET,
	(-1),
};
...
static void mes_sdio_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if(readl(host->base + SDI_CMD) & (1<<STARTCMD)) {
		mrq->cmd->error = -EBUSY;	/* TODO: -ENOMEDIUM? */
		mrq->cmd->retries = 0;
		host->mrq = NULL;
...
static void mes_sdio_setup_pins(struct mes_sdio_host *host, const int on)
{
	...
	for (i = 0; i < 6; i++)
	{
		if(on)
		{
			/* XXX: fix for Arcadyan WiFi
			 * The SDIO physical layer spec says that the SDIO
			 * host must provide a 10k - 100k ohm pull-up on
			 * CMD and DAT[3:0].  Internal pull-ups are 100k ohm.
			 */
...
static void mes_sdio_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	const enum gpio_resource reset = reset_pins[host->channel];
			...
			case MMC_POWER_OFF:
				...
				if (gpio_have_gpio_madrid()) {
					...
					if((int)reset != -1) {
						/* assert reset */
						gpio_configure_pin(
							lf1000_l2p_port(reset),
							lf1000_l2p_pin(reset),
							GPIO_GPIOFN, 1, 0, 0);
					}
				}
				break;
...
static void mes_sdio_reset_card(struct mes_sdio_host *host)
{
	...
	ios.clock = host->clock_hz;
	ios.power_mode = MMC_POWER_OFF;
	mes_sdio_set_ios(host->mmc, &ios);
	mdelay(5);
...
static int mes_sdio_probe(struct platform_device *pdev)
{
	...
	if(gpio_have_gpio_madrid() && (host->channel == 0)) {
